<template>
<div class="row">
    <div class="col">
        <h5>Email notifications</h5>
        <div class="table-responsive">
            <div id="condensedTable_wrapper" class="table table-hover table-condensed">
                <table class="table table-hover table-condensed dataTable no-footer" id="basicTable">
                    <thead>
                        <th style="width:90%">Name</th>
                        <th style="width:10%">Status</th>
                    </thead>
                    <tbody>
                        <tr class="odd">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                        <tr class="even">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                        <tr class="odd">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                        <tr class="even">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                        <tr class="odd">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <h5>Other notifications</h5>
        <div class="table-responsive">
            <div id="condensedTable_wrapper" class="dataTables_wrapper no-footer">
                <table class="table table-hover table-condensed dataTable no-footer" id="basicTable">
                    <thead>
                        <th style="width:95%">Name</th>
                        <th style="width:5%">Status</th>
                    </thead>
                    <tbody>
                        <tr class="odd">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                        <tr class="even">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                        <tr class="odd">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                        <tr class="even">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                        <tr class="odd">
                            <td class="v-align-middle">Lipsum Dolor sit amet</td>
                            <td class="v-align-middle ">
                                <input type="checkbox" data-init-plugin="switchery" data-color="primary" data-size="small" checked="checked" />
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
</template>

<script>
export default {
};
</script>
